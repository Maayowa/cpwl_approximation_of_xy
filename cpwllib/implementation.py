...
def do_product_linearization(
    ...
    if quadratic:
        ...
        model.add_quadratic_constraint(
            (0.0 <= x * y - z) <= 0.0, name=f"z_is_xy({Z_var.name})"
        )
        variables = {"X": X_var, "Y": Y_var, "Z": Z_var, "x": x, "y": y, "z": z}
        return variables

    if partition_method not in ["triangles", "polygons", "sum of convex"]:
        raise ValueError(
            f"Invalid method: {partition_method}. "
    ...
    y = model.add_variable(name=f"{Y_var.name}_aux", lb=0.0, ub=1.0)

    z = model.add_variable(name=f"{Z_var.name}_aux", lb=0.0, ub=1.0)
    ...
    # z = zc_1 + zc_2
    model.add_linear_constraint(
        z == sum(z_c), name=f"sum_convex({X_var.name},{Y_var.name})"
    )
    ...
    for cc in range(N_C):
        ...
        xx_c = [
            model.add_variable(name=f"xx_{X_var.name}_{cc}_{i}", lb=0.0, ub=1.0)
            for i in range(NN)
        ]
        yy_c = [
            model.add_variable(name=f"yy_{Y_var.name}_{cc}_{i}", lb=0.0, ub=1.0)
            for i in range(NN)
        ]
        ...
        if logarithmic_encoding:
            bv_c = [
                model.add_variable(name=f"bv_{Z_var.name}_{cc}_{i}", lb=0.0, ub=1.0)
                for i in range(NN)
            ]
            gv_c = [
                model.add_binary_variable(name=f"gv_{Z_var.name}_{cc}_{q}")
                for q in range(P)
            ]
        else:
            bv_c = [
                model.add_binary_variable(name=f"bv_{Z_var.name}_{cc}_{i}")
                for i in range(NN)
            ]
        ...
        model.add_linear_constraint(
            z_c[cc] == sum(zz_c), name=f"sum_zz_{Z_var.name}_{cc}"
        )
        ...
        for i in range(NN):
            ...
            model.add_linear_constraint(
                xx_c[i] <= bv_c[i], name=f"xx_bound_{Z_var.name}_{cc}_{i}"
            )
            model.add_linear_constraint(
                yy_c[i] <= bv_c[i], name=f"yy_bound_{Z_var.name}_{cc}_{i}"
            )
            for j in range(len(equations_[i])):
                aa, bb, cc_coef = equations_[i][j]
                # inequalities defining the region
                model.add_linear_constraint(
                    aa * xx_c[i] + bb * yy_c[i] + cc_coef * bv_c[i] <= 0.0,

...
class Model:
    ...
    def populate_from_inputs(self, inputs):
        ...
        parameters["LMP"] = {
            (n, t): value
            for n, row in inputs["LMP_values"].iterrows()
            for t, value in row.items()
        }
        parameters["T_gg_hourly_max"] = {
            (g, t): value
            for g, row in inputs["T_gauge_hourly_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["T_gg_hourly_min"] = {
            (g, t): value
            for g, row in inputs["T_gauge_hourly_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["dT_hourly"] = {
            (i, j, t): (
                inputs["dT_hourly_values"].loc[f"{i} - {j}", t]
                if f"{i} - {j}" in inputs["dT_hourly_values"].index
                else 0
            )
            for i in inputs["river_nodes"]
            for j in inputs["river_nodes"]
            for t in inputs["time_horizon"]
        }
        # parameters['T_gg_daily'] = {(g, s): value for g, row in inputs['T_gauge_daily'].iterrows() for s, value in row.items()}
        parameters["T_gg_daily"] = {
            (g, d): value
            for g, row in inputs["T_gauge_daily"].iterrows()
        ...
        parameters["dT_daily"] = {
            (i, j): (
                inputs["dT_daily"].loc[f"{i} - {j}"].tolist()
                if f"{i} - {j}" in inputs["dT_daily"].index
                else [0] * len(inputs["dT_daily"].columns)
            )
            for i in inputs["river_nodes"]
            for j in inputs["river_nodes"]
        }
        parameters["T_bp"] = {
            (p, t): value
            for p, row in inputs["T_bp_values"].iterrows()
            for t, value in row.items()
        }
        parameters["T_tur"] = {
            (p, t): value
            for p, row in inputs["T_tur_values"].iterrows()
            for t, value in row.items()
        }
        parameters["T_source_hourly_max"] = {
            (p, t): (
                inputs["T_source_hourly_max_values"].loc[f"{p}", t]
                if f"{p}" in inputs["T_source_hourly_max_values"].index
                else 25
            )
            for p in inputs["river_nodes"]
            for t in inputs["time_horizon"]
        }
        parameters["T_source_hourly_min"] = {
            (p, t): (
                inputs["T_source_hourly_min_values"].loc[f"{p}", t]
                if f"{p}" in inputs["T_source_hourly_min_values"].index
                else 0
            )
            for p in inputs["river_nodes"]
            for t in inputs["time_horizon"]
        }
        parameters["T_sink_hourly_max"] = {
            (p, t): (
                inputs["T_sink_hourly_max_values"].loc[f"{p}", t]
                if f"{p}" in inputs["T_sink_hourly_max_values"].index
                else 30
            )
            for p in inputs["river_nodes"]
            for t in inputs["time_horizon"]
        }
        parameters["T_sink_hourly_min"] = {
            (p, t): (
                inputs["T_sink_hourly_min_values"].loc[f"{p}", t]
                if f"{p}" in inputs["T_sink_hourly_min_values"].index
                else 0
            )
            for p in inputs["river_nodes"]
            for t in inputs["time_horizon"]
        ...
        parameters["Q_bp_min"] = {
            (p, t): value
            for p, row in inputs["Q_bp_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_bp_max"] = {
            (p, t): value
            for p, row in inputs["Q_bp_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tur_min"] = {
            (p, t): value
            for p, row in inputs["Q_tur_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tur_max"] = {
            (p, t): value
            for p, row in inputs["Q_tur_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tot_min"] = {
            (p, t): value
            for p, row in inputs["Q_tot_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tot_max"] = {
            (p, t): value
            for p, row in inputs["Q_tot_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_gg_min"] = {
            (p, t): value
            for p, row in inputs["Q_gg_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_gg_max"] = {
            (p, t): value
            for p, row in inputs["Q_gg_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_source_max"] = {
            (p, t): value
            for p, row in inputs["Q_source_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_sink_max"] = {
            (p, t): value
            for p, row in inputs["Q_sink_max"].iterrows()
            for t, value in row.items()
        }
        parameters["P_tur_min"] = {
            (p, t): value
            for p, row in inputs["P_tur_min_values"].iterrows()
            for t, value in row.items()
        }
        parameters["P_tur_max"] = {
            (p, t): value
            for p, row in inputs["P_tur_max_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Demand"] = {
            (n, t): value
            for n, row in inputs["Demand_values"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_bp_ramp_up_max"] = {
            (p, t): value
            for p, row in inputs["Q_bp_ramp_up_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_bp_ramp_down_max"] = {
            (p, t): value
            for p, row in inputs["Q_bp_ramp_down_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tur_ramp_up_max"] = {
            (p, t): value
            for p, row in inputs["Q_tur_ramp_up_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tur_ramp_down_max"] = {
            (p, t): value
            for p, row in inputs["Q_tur_ramp_down_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tot_ramp_up_max"] = {
            (p, t): value
            for p, row in inputs["Q_tot_ramp_up_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_tot_ramp_down_max"] = {
            (p, t): value
            for p, row in inputs["Q_tot_ramp_down_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_gg_ramp_up_max"] = {
            (p, t): value
            for p, row in inputs["Q_gg_ramp_up_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_gg_ramp_down_max"] = {
            (p, t): value
            for p, row in inputs["Q_gg_ramp_down_max"].iterrows()
            for t, value in row.items()
        }
        parameters["Flow_i_j_max"] = {
            (i, j, t): (
                inputs["Flow_i_j_max"].loc[f"{i} - {j}", t]
                if f"{i} - {j}" in inputs["Flow_i_j_max"].index
                else 0
            )
            for i in inputs["river_nodes"]
            for j in inputs["river_nodes"]
        ...
        parameters["Flow_i_j_min"] = {
            (i, j, t): (
                inputs["Flow_i_j_min"].loc[f"{i} - {j}", t]
                if not inputs["Flow_i_j_min"].empty
                and f"{i} - {j}" in inputs["Flow_i_j_min"].index
                else 0
            )
            for i in inputs["river_nodes"]
            for j in inputs["river_nodes"]
        ...
        parameters["Plants_to_nodes"] = {
            (p, n): value
            for p, row in inputs["plants_nodes"].iterrows()
            for n, value in row.items()
        }
        parameters["C_bp_ramp_up"] = {
            (p, s): value
            for p, row in inputs["C_bp_ramp_up"].iterrows()
            for s, value in row.items()
        }
        parameters["C_bp_ramp_down"] = {
            (p, s): value
            for p, row in inputs["C_bp_ramp_down"].iterrows()
            for s, value in row.items()
        }
        parameters["C_tur_ramp_up"] = {
            (p, s): value
            for p, row in inputs["C_tur_ramp_up"].iterrows()
            for s, value in row.items()
        }
        parameters["C_tur_ramp_down"] = {
            (p, s): value
            for p, row in inputs["C_tur_ramp_down"].iterrows()
            for s, value in row.items()
        }
        parameters["C_tot_ramp_up"] = {
            (p, s): value
            for p, row in inputs["C_tot_ramp_up"].iterrows()
            for s, value in row.items()
        }
        parameters["C_tot_ramp_down"] = {
            (p, s): value
            for p, row in inputs["C_tot_ramp_down"].iterrows()
            for s, value in row.items()
        }
        parameters["C_gg_ramp_up"] = {
            (p, s): value
            for p, row in inputs["C_gg_ramp_up"].iterrows()
            for s, value in row.items()
        }
        parameters["C_gg_ramp_down"] = {
            (p, s): value
            for p, row in inputs["C_gg_ramp_down"].iterrows()
            for s, value in row.items()
        }
        parameters["C_source"] = {
            (p, s): value
            for p, row in inputs["C_source"].iterrows()
            for s, value in row.items()
        }
        parameters["C_sink"] = {
            (p, s): value
            for p, row in inputs["C_sink"].iterrows()
            for s, value in row.items()
        }
        parameters["Q_tot_daily"] = {
            (p, d): value
            for p, row in inputs["Q_tot_daily"].iterrows()
            for d, value in row.items()
        }
        parameters["Q_max_daily_change"] = {
            (p, d): value
            for p, row in inputs["Q_max_daily_change"].iterrows()
            for d, value in row.items()
        }
        parameters["WaterToPowerConversion"] = {
            (p, d): value
            for p, row in inputs["WaterToPowerConversion"].iterrows()
            for d, value in row.items()
        }
        parameters["bp_ramp_up_cyc"] = {
            (p, s): value
            for p, row in inputs["bp_ramp_up_cyc"].iterrows()
            for s, value in row.items()
        }
        parameters["bp_ramp_down_cyc"] = {
            (p, s): value
            for p, row in inputs["bp_ramp_down_cyc"].iterrows()
            for s, value in row.items()
        }
        parameters["bp_ramp_tot_cyc"] = {
            (p, s): value
            for p, row in inputs["bp_ramp_tot_cyc"].iterrows()
            for s, value in row.items()
        }
        parameters["bp_forced_commit"] = {
            (p, t): value
            for p, row in inputs["bp_forced_commit"].iterrows()
            for t, value in row.items()
        }
        parameters["Q_rough_zone_min"] = {
            (p, r): value
            for p, row in inputs["Q_rough_zone_min"].T.iterrows()
            for r, value in row.items()
        }
        parameters["Q_rough_zone_max"] = {
            (p, r): value
            for p, row in inputs["Q_rough_zone_max"].T.iterrows()
            for r, value in row.items()
        }

        def log_sample_summary(logger, name, parameter_dict, sample_size=5):
            logger.debug(
                f"{name} assigned (sample): {list(parameter_dict.values())[:sample_size]} ... (total: {len(parameter_dict)})"
            )
        ...
        log_sample_summary(
            logger, "T_gg_hourly_max values", parameters["T_gg_hourly_max"]
        )
        log_sample_summary(
            logger, "T_gg_hourly_min values", parameters["T_gg_hourly_min"]
        )
        ...
        log_sample_summary(
            logger, "Q_bp_ramp_down_max", parameters["Q_bp_ramp_down_max"]
        )
        log_sample_summary(logger, "Q_tur_ramp_up_max", parameters["Q_tur_ramp_up_max"])
        log_sample_summary(
            logger, "Q_tur_ramp_down_max", parameters["Q_tur_ramp_down_max"]
        )
        log_sample_summary(logger, "Q_tot_ramp_up_max", parameters["Q_tot_ramp_up_max"])
        log_sample_summary(
            logger, "Q_tot_ramp_down_max", parameters["Q_tot_ramp_down_max"]
        )
        log_sample_summary(logger, "Q_gg_ramp_up_max", parameters["Q_gg_ramp_up_max"])
        log_sample_summary(
            logger, "Q_gg_ramp_down_max", parameters["Q_gg_ramp_down_max"]
        )
        ...
        log_sample_summary(
            logger, "Q_max_daily_change", parameters["Q_max_daily_change"]
        )
        log_sample_summary(
            logger, "WaterToPowerConversion", parameters["WaterToPowerConversion"]
        )
        ...
        variables["q_tot_min"] = [
            [
                model.add_variable(
                    lb=min(
                        value
                        for (p, t), value in parameters["Q_tot_min"].items()
                        if p == plant
                    ),
                    ub=max(
                        value
                        for (p, t), value in parameters["Q_tot_max"].items()
                        if p == plant
                    ),
                    name=f"q_tot_min_{plant}_{d}",
                )
                for d in inputs["daily_horizon"]
            ]
            for plant in inputs["river_nodes"]
        ]
        variables["p_tur"] = [
            [
                model.add_variable(
                    lb=parameters["P_tur_min"][p, t],
        ...
        variables["q_bp"] = [
            [
                model.add_variable(
                    lb=0, ub=parameters["Q_bp_max"][p, t], name=f"q_bp_{p}_{t}"
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ...
        variables["q_flow"] = [
            [
                [
                    model.add_variable(
                        lb=parameters["Flow_i_j_min"][i, j, t],
                        ub=parameters["Flow_i_j_max"][i, j, t],
                        name=f"q_flow_{i}_{j}_{t}",
                    )
                    for t in inputs["time_horizon"]
                ]
                for j in inputs["river_nodes"]
        ...
        variables["q_in_flow"] = [
            [
                model.add_variable(lb=0, ub=100000, name=f"q_in_flow_{p}_{t}")
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_out_flow"] = [
            [
                model.add_variable(lb=0, ub=100000, name=f"q_out_flow_{p}_{t}")
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["QT_latent"] = [
            [
                model.add_variable(lb=0, ub=100000, name=f"QT_latent_{p}_{t}")
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["T_gg"] = [
            [
                model.add_variable(
                    lb=parameters["T_gg_hourly_min"][j, t],
                    ub=parameters["T_gg_hourly_max"][j, t],
                    name=f"T_gauge_{j}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for j in inputs["river_nodes"]
        ]
        variables["q_bp_ramp_up"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_bp_ramp_up_max"][p, "Single input"],
                    name=f"q_bp_ramp_up_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_bp_ramp_down"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_bp_ramp_down_max"][p, "Single input"],
                    name=f"q_bp_ramp_down_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_tur_ramp_up"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_tur_ramp_up_max"][p, "Single input"],
                    name=f"q_tur_ramp_up_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_tur_ramp_down"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_tur_ramp_down_max"][p, "Single input"],
                    name=f"q_tur_ramp_down_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_tot_ramp_up"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_tot_ramp_up_max"][p, "Single input"],
                    name=f"q_tot_ramp_up_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_tot_ramp_down"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_tot_ramp_down_max"][p, "Single input"],
                    name=f"q_tot_ramp_down_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_gg_ramp_up"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_gg_ramp_up_max"][p, "Single input"],
                    name=f"q_gg_ramp_up_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["q_gg_ramp_down"] = [
            [
                model.add_variable(
                    lb=0,
                    ub=parameters["Q_gg_ramp_down_max"][p, "Single input"],
                    name=f"q_gg_ramp_down_{p}_{t}",
                )
                for t in inputs["time_horizon"]
            ]
            for p in inputs["river_nodes"]
        ...
        # Slack Variables
        variables["slack_q_tot_daily_min"] = [
            [
                model.add_variable(
                    lb=0, ub=25000, name=f"slack_q_tot_daily_min_{p}_{d}"
                )
                for d in inputs["daily_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_tot_daily_max"] = [
            [
                model.add_variable(
                    lb=0, ub=25000, name=f"slack_q_tot_daily_max_{p}_{d}"
                )
                for d in inputs["daily_horizon"]
            ]
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_bp_cyc_min"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_bp_cyc_min_{p}")
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_bp_cyc_max"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_bp_cyc_max_{p}")
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_tur_cyc_min"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_tur_cyc_min_{p}")
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_tur_cyc_max"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_tur_cyc_max_{p}")
            for p in inputs["river_nodes"]
        ]
        variables["slack_q_tot_cyc_min"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_tot_cyc_min_{p}")
            for p in inputs["river_nodes"]
        ]
        ...
        variables["slack_q_gg_cyc_max"] = [
            model.add_variable(lb=0, ub=25000, name=f"slack_q_gg_cyc_max_{p}")
            for p in inputs["river_nodes"]
        ]
        variables["slack_p_node_min"] = [
            [
                model.add_variable(lb=0, ub=10000, name=f"slack_p_tur_min_{n}_{t}")
                for t in inputs["time_horizon"]
            ]
            for n in inputs["nodes"]
        ]
        variables["slack_p_node_max"] = [
            [
                model.add_variable(lb=0, ub=10000, name=f"slack_p_tur_max_{n}_{t}")
                for t in inputs["time_horizon"]
            ]
            for n in inputs["nodes"]
        ]
        ...
        if inputs["sim_type"] == "MILP":
            variables["q_bp_rr_up_change"] = [
                [
                    model.add_binary_variable(name=f"q_bp_rr_up_change_{p}_{t}")
                    for t in inputs["time_horizon"]
                ]
                for p in inputs["river_nodes"]
            ]
            variables["q_bp_rr_down_change"] = [
                [
                    model.add_binary_variable(name=f"q_bp_rr_down_change_{p}_{t}")
                    for t in inputs["time_horizon"]
                ]
                for p in inputs["river_nodes"]
            ]
            variables["q_bp_committed"] = [
                [
                    model.add_integer_variable(
                        lb=parameters["bp_forced_commit"][p, t],
                        ub=1,
                        name=f"q_bp_committed_{p}_{t}",
                    )
                    for t in inputs["time_horizon"]
                ]
                for p in inputs["river_nodes"]
            ...
            if inputs["cnstr_rough_zone"]:
                variables["q_tur_committed"] = [
                    [
                        [
                            model.add_integer_variable(
                                lb=0, ub=1, name=f"q_tur_committed_{p}_{r}_{t}"
                            )
                            for t in inputs["time_horizon"]
                        ]
        ...
        for p in range(len(inputs["river_nodes"])):
            ...
            for t in range(len(inputs["time_horizon"])):
                ...
                obj_fnc += (
                    sum(
                        variables["p_tur"][p][t]
                        * parameters["LMP"][n, time]
                        * parameters["Plants_to_nodes"][plant, n]
                        for n in inputs["nodes"]
                    )
                    - variables["q_bp_ramp_up"][p][t]
                    * parameters["C_bp_ramp_up"][plant, "Single input"]
                    - variables["q_bp_ramp_down"][p][t]
                    * parameters["C_bp_ramp_down"][plant, "Single input"]
                    - variables["q_tur_ramp_up"][p][t]
                    * parameters["C_tur_ramp_up"][plant, "Single input"]
                    - variables["q_tur_ramp_down"][p][t]
                    * parameters["C_tur_ramp_down"][plant, "Single input"]
                    - variables["q_tot_ramp_up"][p][t]
                    * parameters["C_tot_ramp_up"][plant, "Single input"]
                    - variables["q_tot_ramp_down"][p][t]
                    * parameters["C_tot_ramp_down"][plant, "Single input"]
                    - variables["q_gg_ramp_up"][p][t]
                    * parameters["C_gg_ramp_up"][plant, "Single input"]
                    - variables["q_gg_ramp_down"][p][t]
                    * parameters["C_gg_ramp_down"][plant, "Single input"]
                )
            ...
            obj_fnc += (
                -(
                    variables["slack_q_bp_cyc_min"][p]
                    + variables["slack_q_bp_cyc_max"][p]
                    + variables["slack_q_tur_cyc_min"][p]
                    + variables["slack_q_tur_cyc_max"][p]
                    + variables["slack_q_tot_cyc_min"][p]
                    + variables["slack_q_tot_cyc_max"][p]
                    + variables["slack_q_gg_cyc_min"][p]
                    + variables["slack_q_gg_cyc_max"][p]
                )
                * inputs["C_slack"]
            )
        ...
        for t in range(len(inputs["time_horizon"])):
            obj_fnc += -sum(
                (
                    variables["slack_p_node_min"][n][t]
                    + variables["slack_p_node_max"][n][t]
                )
                * inputs["C_slack"]
                ...
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            sum(
                                variables["q_tur_committed"][p][r][t]
                                * parameters["Q_rough_zone_min"][
                                    plant, inputs["rough_zones"][r]
                                ]
                                for r in range(len(inputs["rough_zones"]))
                            )
                            <= variables["q_tur"][p][t]
                        )
                    )
        ...
        for p in range(len(inputs["river_nodes"])):
            for t in range(len(inputs["time_horizon"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_bp"][p][t] + variables["q_tur"][p][t]
                        == variables["q_tot"][p][t]
                    )
                )
            ...
            for d in range(days_per_week):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        ...
                        # + variables['slack_q_tot_daily_min'][p][d]
                        == parameters["Q_tot_daily"][
                            (inputs["river_nodes"][p], f"D{d + 1:02}")
                        ]
                        # + variables['slack_q_tot_daily_max'][p][d]
                    )
                )
        ...
        if inputs["sim_type"] == "MILP":
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    constraint_id_main.append(
                        "Hourly bypass ramp up change status upper bound"
                    )
                    constraint_id_unit.append(f"{inputs['river_nodes'][p]}")
                    constraint_id_time.append(f"{inputs['time_horizon'][t]}")
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_bp_ramp_up"][p][t]
                            <= variables["q_bp_rr_up_change"][p][t]
                            * parameters["Q_bp_ramp_up_max"][
                                inputs["river_nodes"][p], "Single input"
                            ],
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    constraint_id_main.append(
                        "Hourly bypass ramp down change status upper bound"
                    )
                    constraint_id_unit.append(f"{inputs['river_nodes'][p]}")
                    constraint_id_time.append(f"{inputs['time_horizon'][t]}")
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_bp_ramp_down"][p][t]
                            <= variables["q_bp_rr_down_change"][p][t]
                            * parameters["Q_bp_ramp_down_max"][
                                inputs["river_nodes"][p], "Single input"
                            ],
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_bp_committed"][p][t]
                            * parameters["Q_bp_max"][
                                inputs["river_nodes"][p], inputs["time_horizon"][t]
                            ]
                            >= variables["q_bp"][p][t]
                        )
                    )
        ...
        for i in range(len(inputs["river_nodes"])):
            for t in range(len(inputs["time_horizon"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_out_flow"][i][t]
                        == sum(
                            variables["q_flow"][i][j][t]
        ...
        for i in range(len(inputs["river_nodes"])):
            for t in range(len(inputs["time_horizon"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_in_flow"][i][t]
                        == sum(
                            variables["q_flow"][j][i][t]
                            for j in range(len(inputs["river_nodes"]))
                        )
                    )
                )
        ...
        if inputs["cnstr_bp_t01t24"]:
            logger.info(
                "Bypass release cyclic constraints (H01=H24) have been included in the optimization."
            )
            if inputs["cnstr_temp_t01t24"]:
                logger.warning(
                    "Bypass release cyclic constraints (H01=H24) may conflict with cyclic temperature constraints."
                )
            for p in range(len(inputs["river_nodes"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_bp"][p][0] + variables["slack_q_bp_cyc_min"][p]
                        == variables["q_bp"][p][len(inputs["time_horizon"]) - 1]
                        + variables["slack_q_bp_cyc_max"][p]
                    )
                )
        ...
        if inputs["cnstr_tur_t01t24"]:
            logger.info(
                "Turbine release cyclic constraints (H01=H24) have been included in the optimization."
            )
            if inputs["cnstr_temp_t01t24"]:
                logger.warning(
                    "Turbine release cyclic constraints (H01=H24) may conflict with cyclic temperature constraints."
                )
            for p in range(len(inputs["river_nodes"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_tur"][p][0] + variables["slack_q_tur_cyc_min"][p]
                        == variables["q_tur"][p][len(inputs["time_horizon"]) - 1]
                        + variables["slack_q_tur_cyc_max"][p]
                    )
                )
        ...
        if inputs["cnstr_tot_t01t24"]:
            logger.info(
                "Total release cyclic constraints (H01=H24) have been included in the optimization."
            )
            if inputs["cnstr_temp_t01t24"]:
                logger.warning(
                    "Total release cyclic constraints (H01=H24) may conflict with cyclic temperature constraints."
                )
            for p in range(len(inputs["river_nodes"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_tot"][p][0] + variables["slack_q_tot_cyc_min"][p]
                        == variables["q_tot"][p][len(inputs["time_horizon"]) - 1]
                        + variables["slack_q_tot_cyc_max"][p]
                    )
                )
        ...
        if inputs["cnstr_gg_t01t24"]:
            logger.info(
                "Gauge release cyclic constraints (H01=H24) have been included in the optimization."
            )
            if inputs["cnstr_temp_t01t24"]:
                logger.warning(
                    "Gauge release cyclic constraints (H01=H24) may conflict with cyclic temperature constraints."
                )
            for p in range(len(inputs["river_nodes"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_gg"][p][0] + variables["slack_q_gg_cyc_min"][p]
                        == variables["q_gg"][p][len(inputs["time_horizon"]) - 1]
                        + variables["slack_q_gg_cyc_min"][p]
                    )
                )
        ...
        if inputs["cnstr_oper_range"] is True:
            logger.info(
                "Maximum daily operating range constraints have been included in the optimization."
            )
            ...
            for p in range(len(inputs["river_nodes"])):
                for d in range(days_per_week):
                    ...
                    for t in range(start_hour, end_hour):
                        ...
                        constraints.append(
                            model.add_linear_constraint(
                                variables["q_tot_min"][p][d] <= variables["q_tot"][p][t]
                            )
                        )
            # Add constraints for maximum total daily ramping rate 8000CFS
            for p in range(len(inputs["river_nodes"])):
        ...
        if inputs["cnstr_bp_ramp"] is True:
            logger.info(
                "Bypass release ramping constraints have been included in the optimization."
            )
            # Add constraint for bypass ramping up rate
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    prev_t = (
                        t - 1 if t > 0 else len(inputs["time_horizon"]) - 1
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_bp"][p][t] - variables["q_bp"][p][prev_t]
                            <= variables["q_bp_ramp_up"][p][t]
                        )
                    )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_bp"][p][prev_t] - variables["q_bp"][p][t]
                            <= variables["q_bp_ramp_down"][p][t]
                        )
                    )
        ...
        if inputs["cnstr_tur_ramp"] is True:
            logger.info(
                "Turbine release ramping constraints have been included in the optimization."
            )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_tur"][p][t] - variables["q_tur"][p][prev_t]
                            <= variables["q_tur_ramp_up"][p][t]
                        )
                    )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_tur"][p][prev_t] - variables["q_tur"][p][t]
                            <= variables["q_tur_ramp_down"][p][t]
                        )
                    )
        ...
        if inputs["cnstr_tot_ramp"] is True:
            logger.info(
                "Total release ramping constraints have been included in the optimization."
            )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_tot"][p][t] - variables["q_tot"][p][prev_t]
                            <= variables["q_tot_ramp_up"][p][t]
                        )
                    )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_tot"][p][prev_t] - variables["q_tot"][p][t]
                            <= variables["q_tot_ramp_down"][p][t]
                        )
                    )
        ...
        if inputs["cnstr_gg_ramp"] is True:
            logger.info(
                "Gauge release ramping constraints have been included in the optimization."
            )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_gg"][p][t] - variables["q_gg"][p][prev_t]
                            <= variables["q_gg_ramp_up"][p][t]
                        )
                    )
            ...
            for p in range(len(inputs["river_nodes"])):
                for t in range(len(inputs["time_horizon"])):
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            variables["q_gg"][p][prev_t] - variables["q_gg"][p][t]
                            <= variables["q_gg_ramp_down"][p][t]
                        )
                    )
        ...
        for n in range(len(inputs["nodes"])):
            ...
            for t in range(len(inputs["time_horizon"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        parameters["Demand"][node, time]
                        + variables["slack_p_node_min"][n][t]
                        - variables["slack_p_node_max"][n][t]
                        - sum(
                            variables["p_tur"][p][t]
                            * parameters["Plants_to_nodes"][
                                inputs["river_nodes"][p], node
                            ]
                            for p in range(len(inputs["river_nodes"]))
                        )
                        >= 0
        ...
        for p in range(len(inputs["river_nodes"])):
            ...
            for t in range(len(inputs["time_horizon"])):
                ...
                constraints.append(
                    model.add_linear_constraint(
                        variables["q_tur"][p][t] * parameters["T_tur"][plant, time]
                        + variables["q_bp"][p][t] * parameters["T_bp"][plant, time]
                        + variables["QT_latent"][p][t]
                        <= variables["q_sink"][p][t]
                        * parameters["T_sink_hourly_max"][
                            inputs["river_nodes"][p], time
                        ]
                        + sum(
                            (
                                parameters["T_gg_hourly_max"][
                                    inputs["river_nodes"][j], time
                                ]
                                - parameters["dT_hourly"][
                                    inputs["river_nodes"][p],
                                    inputs["river_nodes"][j],
                                    time,
                                ]
                            )
                            * variables["q_flow"][p][j][t]
                            for j in range(len(inputs["river_nodes"]))
                        ),
                    ...
                    model.add_linear_constraint(
                        ...
                        * sum(
                        ...
                        + sum(
                            (
                                parameters["T_gg_daily"][
                                    inputs["river_nodes"][j], f"D{d + 1:02}"
                                ]
                                - parameters["dT_daily"][
                                    inputs["river_nodes"][p], inputs["river_nodes"][j]
                                ][d]
                            )
                            * sum(
                                variables["q_flow"][p][j][t]

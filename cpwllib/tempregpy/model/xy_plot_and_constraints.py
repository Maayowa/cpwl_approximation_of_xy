...
def reshape_list(flat_list, shape):
    ...
    if total_elements != product_of_shape:
        raise ValueError(
            "The number of elements in the flat_list does not match the total elements in the specified shape."
        )
...
def evaluate_gn(x_y_array, N):
    ...
    mat_minus = np.c_[
        -(2 * (k_vec - N) + 1) / (4 * N),
        (2 * (k_vec - N) + 1) / (4 * N),
        -(k_vec * (k_vec + 1) - 2 * N * k_vec - N * (1 - N)) / (4 * N**2),
    ]
...
def MILP_or_QP_variables_and_constraints_hydro(
    ...
    if partition_method == "quadratic":
        pass
    elif partition_method not in ["triangles", "polygons", "sum of convex"]:
    ...
    plants = list(X.keys())
    N_units = {p: len(X[p]) for p in X}
    N_time = len(next(iter(Y.values())))

    # # initial variables for Z = X*Y (Q and h)
    # X = model.add_variable(name="X")
    # Y = model.add_variable(name="Y")
    Z = {
        p: [
            [model.add_variable(name=f"Z({p},{u},{t})") for t in range(N_time)]
    ...
    x = {
        p: [
            [
                model.add_variable(name=f"x({p},{u},{t})", lb=0.0, ub=1.0)
    ...
    y = {
        p: [
            model.add_variable(name=f"y({p},{t})", lb=0.0, ub=1.0)
            for t in range(N_time)
        ]
        for p in plants
    }
    z = {
        p: [
            [
                model.add_variable(name=f"z({p},{u},{t})", lb=0.0, ub=1.0)
                for t in range(N_time)
            ]
            for u in range(N_units[p])
        ]
    ...
    if quadratic:
        ...
        z_c = {
            p: [[[] for t in range(N_time)] for u in range(N_units[p])] for p in plants
        }

        xx = {
            p: [[[] for t in range(N_time)] for u in range(N_units[p])] for p in plants
        }
        yy = {p: [[] for t in range(N_time)] for p in plants}
        zz = {
            p: [[[] for t in range(N_time)] for u in range(N_units[p])] for p in plants
        }
        bv = {
        ...
        if partition_method in ["triangles", "polygons"]:
            ...
            N_C = 2
            # get linear coeffs and region inequalities
            list_coeffs_j, list_equations_j, list_coeffs_k, list_equations_k = (
                equations_sum_convex(N)
            )
    ...
    for p in plants:
        for u in range(N_units[p]):
            for t in range(N_time):
                ...
                X_min = X[p][u][t].lower_bound
                X_max = X[p][u][t].upper_bound

                # link between initial and rescaled variables
                model.add_linear_constraint(
                    X[p][u][t] == X_min + (X_max - X_min) * x[p][u][t],
                    name=f"X_to_x({p},{u},{t})",
                )
                if u == 0:
                    ...
                    model.add_linear_constraint(
                        Y[p][t] == Y_min + (Y_max - Y_min) * y[p][t],
                        name=f"Y_to_y({p},{t})",
                    )
                model.add_linear_constraint(
                    Z[p][u][t]
                    == X_min * Y_min
                    + Y_min * (X_max - X_min) * x[p][u][t]
                    + X_min * (Y_max - Y_min) * y[p][t]
                    + (Y_max - Y_min) * (X_max - X_min) * z[p][u][t],
                    name=f"Z_to_z({p},{u},{t})",
                )
                ...
                if quadratic:
                    ...
                    model.add_quadratic_constraint(
                        (0.0 <= x[p][u][t] * y[p][t] - z[p][u][t]) <= 0.0,
                        name=f"z_is_xy({p},{u},{t})",
                    )
                    ...
                    z_c[p][u][t] = [
                        model.add_variable(name=f"z_c({p},{u},{t},{cc})")
                        for cc in range(N_C)
                    ]
                    # z = zc_1 + zc_2
                    model.add_linear_constraint(
                        z[p][u][t] == sum(z_c[p][u][t]), name=f"sum_convex({p},{u},{t})"
                    )
                    ...
                    for cc in range(N_C):
                        ...
                        xx_c = [
                            model.add_variable(
                                name=f"xx({p},{u},{t},{cc},{i})", lb=0.0, ub=1.0
                            )
                            for i in range(NN)
                        ]
                        yy_c = [
                            model.add_variable(
                                name=f"yy({p},{t},{cc},{i})", lb=0.0, ub=1.0
                            )
                            for i in range(NN)
                        ]
                        zz_c = [
                            model.add_variable(name=f"zz({p},{u},{t},{cc},{i})")
                            for i in range(NN)
                        ]
                        ...
                        if logarithmic_encoding:
                            bv_c = [
                                model.add_variable(
                                    name=f"bv({p},{u},{t},{cc},{i})", lb=0.0, ub=1.0
                                )
                                for i in range(NN)
                            ]
                            gv_c = [
                                model.add_binary_variable(
                                    name=f"gv({p},{u},{t},{cc},{q})"
                                )
                                for q in range(P)
                            ]
                        else:
                            bv_c = [
                                model.add_binary_variable(
                                    name=f"bv({p},{u},{t},{cc},{i})"
                                )
                                for i in range(NN)
                            ]

                        xx[p][u][t].append(xx_c)
                        yy[p][t].append(yy_c)
                        zz[p][u][t].append(zz_c)
                        bv[p][u][t].append(bv_c)

                        if logarithmic_encoding:
                            gv[p][u][t].append(gv_c)

                        # x, y, z are equal to the sum of their components
                        model.add_linear_constraint(
                            x[p][u][t] == sum(xx_c), name=f"sum_xx({p},{u},{t},{cc})"
                        )
                        model.add_linear_constraint(
                            y[p][t] == sum(yy_c), name=f"sum_yy({p},{t},{cc})"
                        )
                        model.add_linear_constraint(
                            z_c[p][u][t][cc] == sum(zz_c),
                            name=f"sum_zz({p},{u},{t},{cc})",
                        )
                        # only one region can be active at a time
                        model.add_linear_constraint(
                            sum(bv_c) == 1.0, name=f"sum_bv({p},{u},{t},{cc})"
                        )

                        if logarithmic_encoding:
                            binary_array = np.array(
                                [binary_rep(i, NN) for i in range(NN)]
                            )
                            # TEST
                            # model.add_linear_constraint(
                            #     sum([bv_c[i]*i for i in range(NN)]) == sum([gv_c[q]*(2**q) for q in range(P)]),
                            #     name=f"bv_gv_link({p},{u},{t},{cc})")
                            for q in range(P):
                                model.add_linear_constraint(
                                    sum(
                                        [
                                            bv_c[i]
                                            for i in range(NN)
                                            if binary_array[i, q] == 1
                                        ]
                                    )
                                    <= gv_c[q],
                                    name=f"gv_lb({p},{u},{t},{cc},{q})",
                                )
                                model.add_linear_constraint(
                                    sum(
                                        [
                                            bv_c[i]
                                            for i in range(NN)
                                            if binary_array[i, q] == 0
                                        ]
                                    )
                                    <= 1 - gv_c[q],
                                    name=f"gv_ub({p},{u},{t},{cc},{q})",
                                )
                        for i in range(NN):
                            a, b, c = coeffs[i]
                            # relationship between x,y,z on a given region
                            model.add_linear_constraint(
                                zz_c[i] == a * xx_c[i] + b * yy_c[i] + c * bv_c[i]
                            )
                            ...
                            for j in range(len(equations[i])):
                                ...
                                model.add_linear_constraint(
                                    aa * xx_c[i] + bb * yy_c[i] + cc * bv_c[i] <= 0.0
                                )

    if quadratic:
        variables = {"X": X, "Y": Y, "Z": Z, "x": x, "y": y, "z": z}
    else:
        variables = {
            "X": X,
            "Y": Y,
...
def list_faces_from_N_DC(N):
    ...
    for j in range(2 * N):
        ...
        if j == 0:
            ...
            pts.append(
                [(j - N) / N, 1, evaluate_gn(np.array([[(j - N) / N, 1]]), N)[0][0]]
            )
            pts.append(
                [1, (j - N) / N, evaluate_gn(np.array([[1, (j - N) / N]]), N)[0][0]]
            )
        if j == 2 * N - 1:
            pts.append([1, 1, evaluate_gn(np.array([[1, 1]]), N)[0][0]])
        elif j < N:
            pts.append(
                [(j + 1) / N, 0, evaluate_gn(np.array([[(j + 1) / N, 0]]), N)[0][0]]
            )
            pts.append(
                [0, (j + 1) / N, evaluate_gn(np.array([[0, (j + 1) / N]]), N)[0][0]]
            )
        else:
            pts.append(
                [
                    1,
                    (j + 1 - N) / N,
                    evaluate_gn(np.array([[1, (j + 1 - N) / N]]), N)[0][0],
                ]
            )
            pts.append(
                [
                    (j + 1 - N) / N,
                    1,
                    evaluate_gn(np.array([[(j + 1 - N) / N, 1]]), N)[0][0],
                ]
            )
        pts = np.array(pts)
    ...
    for k in range(2 * N):
        ...
        if k == 0:
            ...
            pts.append(
                [0, (k - N) / N, evaluate_gn(np.array([[0, (k - N) / N]]), N)[1][0]]
            )
            pts.append(
                [
                    1 - (k - N) / N,
                    1,
                    evaluate_gn(np.array([[1 - (k - N) / N, 1]]), N)[1][0],
                ]
            )
        if k == 2 * N - 1:
            ...
            pts.append(
                [1, (k + 1) / N, evaluate_gn(np.array([[1, (k + 1) / N]]), N)[1][0]]
            )
            pts.append(
                [
                    1 - (k + 1) / N,
                    0,
                    evaluate_gn(np.array([[1 - (k + 1) / N, 0]]), N)[1][0],
                ]
            )
        else:
            pts.append(
                [
                    1 - (k + 1 - N) / N,
                    1,
                    evaluate_gn(np.array([[1 - (k + 1 - N) / N, 1]]), N)[1][0],
                ]
            )
            pts.append(
                [
                    0,
                    (k + 1 - N) / N,
                    evaluate_gn(np.array([[0, (k + 1 - N) / N]]), N)[1][0],
                ]
            )
        pts = np.array(pts)
    ...
    if isinstance(faces, tuple):
        for i, f in enumerate(faces):
            faceCollection = Poly3DCollection(
                f, shade=False, facecolors=f"C{i}", edgecolors="k", alpha=0.6
            )
            ax.add_collection3d(faceCollection)
    else:
        faceCollection = Poly3DCollection(
            faces, shade=False, facecolors="C1", edgecolors="k", alpha=0.5
        )
        ax.add_collection3d(faceCollection)
...
def plot_faces_2d(faces):
    ...
    faceCollection = PolyCollection(
        [face[:, :2] for face in faces], facecolors="white", edgecolors="k"
    )
...
def MILP_or_QP_variables_and_constraints(
    model,
    X,
    Y,
    Z,
    ...
    # Y = model.add_variable(name="Y")
    # Z = [
    #     [model.add_variable(name=f"Z({p},{t})") for t in range(N_time)]
    ...
    # rescaled variables
    x = [
        [model.add_variable(name=f"x({p},{t})", lb=0.0, ub=1.0) for t in range(N_time)]
        for p in range(N_nodes)
    ]
    y = [
        [model.add_variable(name=f"y({p},{t})", lb=0.0, ub=1.0) for t in range(N_time)]
        for p in range(N_nodes)
    ]
    z = [
        [model.add_variable(name=f"z({p},{t})", lb=0.0, ub=1.0) for t in range(N_time)]
        for p in range(N_nodes)
    ]
        ...
        for t in range(N_time):
            ...
            constraints.append(
                model.add_linear_constraint(
                    Z[p][t]
                    == X_min * Y_min
                    + Y_min * (X_max - X_min) * x[p][t]
                    + X_min * (Y_max - Y_min) * y[p][t]
                    + (Y_max - Y_min) * (X_max - X_min) * z[p][t],
                    name=f"Z_to_z({p},{t})",
                )
            )
            ...
            if quadratic:
                ...
                # z_c = [model.add_variable(name=f"z_c({cc})") for cc in range(N_C)]
                z_c[p][t] = [
                    model.add_variable(name=f"z_c({p},{t},{cc})") for cc in range(N_C)
                ]
                # z = zc_1 + zc_2

                constraints.append(
                    model.add_linear_constraint(
                        z[p][t] == sum(z_c[p][t]), name=f"sum_convex({p},{t})"
                    )
                )
                ...
                for cc in range(N_C):
                    ...
                    # define regional components of x, y, z
                    xx_c = [
                        model.add_variable(name=f"xx({p},{t},{cc},{i})", lb=0.0, ub=1.0)
                        for i in range(NN)
                    ]
                    yy_c = [
                        model.add_variable(name=f"yy({p},{t},{cc},{i})", lb=0.0, ub=1.0)
                        for i in range(NN)
                    ]
                    zz_c = [
                        model.add_variable(name=f"zz({p},{t},{cc},{i})")
                        for i in range(NN)
                    ]
                    # binary variable indicating which region is active
                    if logarithmic_encoding:
                        bv_c = [
                            model.add_variable(
                                name=f"bv({p},{t},{cc},{i})", lb=0.0, ub=1.0
                            )
                            for i in range(NN)
                        ]
                        gv_c = [
                            model.add_binary_variable(name=f"gv({p},{t},{cc},{q})")
                            for q in range(P)
                        ]
                    else:
                        bv_c = [
                            model.add_binary_variable(name=f"bv({p},{t},{cc},{i})")
                            for i in range(NN)
                        ]
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            x[p][t] == sum(xx_c), name=f"sum_xx({p},{t},{cc})"
                        )
                    )
                    constraints.append(
                        model.add_linear_constraint(
                    ...
                    constraints.append(
                        model.add_linear_constraint(
                            z_c[p][t][cc] == sum(zz_c), name=f"sum_zz({p},{t},{cc})"
                        )
                    )
                    # only one region can be active at a time
                    constraints.append(
                        model.add_linear_constraint(
                            sum(bv_c) == 1.0, name=f"sum_bv({p},{t},{cc})"
                        )
                    )

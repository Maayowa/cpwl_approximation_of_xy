...
def parse_model_results(inputs, result, variables):
    ...
    for var in ["q_tot_min"]:
        results[var] = pd.DataFrame(
            columns=inputs["river_nodes"], index=inputs["daily_horizon"]
        )
        for p in range(len(inputs["river_nodes"])):
            ...
            for d in range(len(inputs["daily_horizon"])):
                ...
                results[var].loc[day, plant] = (
                    result.variable_values(variables[var][p][d]) / CFS_TO_AFH
                )
    ...
    for var in [
        ...
        results[var] = {}
        for p in range(len(inputs["river_nodes"])):
            results[var][inputs["river_nodes"][p]] = result.variable_values(
                variables[var][p]
            )
        results[var] = (
            pd.DataFrame(results[var], index=inputs["time_horizon"]) / CFS_TO_AFH
        )
    ...
    for var in ["q_flow"]:
        ...
        for i in range(len(inputs["river_nodes"])):
            for j in range(len(inputs["river_nodes"])):
                results[var][inputs["river_nodes"][i], inputs["river_nodes"][j]] = (
                    result.variable_values(variables[var][i][j])
                )
    ...
    results["T_plant"] = (
        results["q_bp"] * inputs["T_bp_values"].T
        + results["q_tur"] * inputs["T_tur_values"].T
    ) / (results["q_bp"] + results["q_tur"])
        ...
        for time_interval in t_plant_df.index:
            ...
            for col in q_flow_df.columns:
                ...
                if from_node in t_plant:
                    t_gauge[to_node] = (
                        t_plant[from_node]
                        + dt_hourly_values_df.at[
                            f"{from_node} - {to_node}", time_interval
                        ]
                    )
    ...
    t_gauge = compute_gauge_temperatures(
        results["T_plant"], results["q_flow"], inputs["dT_hourly_values"]
    )
    ...
    for var in [
        ...
        results[var] = pd.DataFrame(
            result.variable_values(variables[var]), index=inputs["river_nodes"]
        )

    # Slack variables
    for var in ["slack_q_tot_daily_min", "slack_q_tot_daily_max"]:
        results[var] = pd.DataFrame(
            index=inputs["daily_horizon"], columns=inputs["river_nodes"]
        )
        for p in range(len(inputs["river_nodes"])):
            plant = inputs["river_nodes"][p]
